...
// FIX 1: API URL টি সঠিকভাবে VITE Environment Variable থেকে নিতে হবে।
// যদি VITE ব্যবহার না করে React App তৈরি করে থাকো, তবে এটি window.location.origin হতে পারে।
// যেহেতু তুমি VITE_API_URL ব্যবহার করছো, আমরা ধরে নিচ্ছি এটি VITE থেকে লোড হয়।
// কিন্তু যেহেতু এটি একটি একক ফাইল, আমরা পুরো Render URL টি ব্যবহার করব।
// CRITICAL FIX: The API_URL was previously containing 'VITE_API_URL=' in the string.
const API_BASE_URL = 'https://personal-server-uf48.onrender.com';
...
const BlogPage = () => {
    ...
    useEffect(() => {
        const fetchBlogs = async () => {
            try {
                setLoading(true);
                // FIX 2: Correcting the API call path. Your back-end route is /api/blogs, so we need to append /api/blogs
                const response = await fetch(`${API_BASE_URL}/api/blogs`); 

                if (!response.ok) {
                    // FIX 3: More robust error handling for 404/500
                    ...
                    try {
                        ...
                        throw new Error(errorData.message || `Server Error: Status ${response.status}`);
                         ...
                         // Fallback if the response is not valid JSON (e.g., plain 404 page, which causes "Unexpected end of JSON input")
                         throw new Error(`Failed to load blogs. Status: ${response.status}. Check server logs.`);
                    }
                }
                ...
                setError(err.message || "Failed to load blog posts. Please check the server connection.");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <section className="min-h-screen py-24 px-4 md:px-16 bg-[#1f242d] text-white">
             ...
             <div className="container mx-auto">
                 ...
                 {blogPosts.length === 0 ? (
                     <p className="text-center text-gray-400 text-xl py-12">No published blog posts found. Please publish content from the admin panel.</p>
                 ) : (
                     <motion.div
                         initial={{ opacity: 0 }}
                         animate={{ opacity: 1 }}
