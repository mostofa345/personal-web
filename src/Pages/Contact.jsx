...
// CRITICAL FIX: The API_BASE_URL was previously containing 'VITE_API_URL=' in the string.
...
const Contact = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // FIX: Correctly forming the full API URL for message submission
            ...
            // FIX: Always attempt to read the response body, even if the status is not ok, to get the error message
            // Note: If the server sends a 204 No Content, response.json() will still fail. 
            // Since your backend sends 201 with JSON, we expect a body.
            const data = await response.json(); 
            
            if (response.ok && data.success) {
                // Success case: Server returned 200/201 and success: true
                setSubmitStatus({
                    loading: false,
                    success: true,
                ...
                // Error case: Server returned a non-200/201 status OR success: false in the body
                setSubmitStatus({
                    loading: false,
                    success: false,
                    message: data.message || `Failed to send message. Server responded with status ${response.status}.`,
                });
            }
        } catch (error) {
            ...
            setSubmitStatus({
                loading: false,
                success: false,
                message: "A network error occurred or server response was invalid. Please try again later.",
            });
        }
    };
